...
function Login() {
    ...
    const {setUser , setToken} = useContext(UserData);
    ...
    async function submitHandler(data , event) {
        ...
        try{
            ...
            const response = await apiConnector("POST" , userEndpoints.User_login_api , data);
                ...
                <form onSubmit={handleSubmit(submitHandler)} className="flex flex-col gap-9">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className='w-full relative'>
                            ...
                            <span className="absolute right-3 top-[17px]" onClick={() => setShowPassword((prev) => !prev)}>
                    ...
                    <NavLink to={'/forgot-password'}><p className="text-lg -mt-7 text-blue-500 text-right">Forgot Password</p></NavLink>

                    <button disabled={loading} className="bg-[#111] text-white font-semibold mb-3 rounded-lg px-4 py-4 w-full text-2xl">Login</button>

                </form>

                <p className='text-center -mt-8 text-lg'>New Here? <NavLink to={'/signup'} className='text-blue-600'>Create account</NavLink></p>

            </div>

            <div className='flex flex-col gap-3'>
                <NavLink to={'/captain-login'}>
                    <button disabled={loading} className="bg-[#10b461] text-white font-semibold mb-3 rounded-lg px-4 py-4 w-full text-2xl">Sign in as Captain</button>
                </NavLink>
